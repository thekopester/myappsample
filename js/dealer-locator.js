...
// necessary for the use of promises
'use strict'
...
const OLD_LIST = "#location-list"; // if you need to change this, please also do a search for this ID and replace those as well
const SEARCH_TEXT = "#postSearch";
const LOCATIONS_FILE = "/js/locations.js";
const STARTING_POSITION = { lat: 39.8097343, lng: -98.5556199 } // set the starting position and reset button of the map
const MAP_ZOOM = 4; // initial zoom of map - set this to a numeric value - the higher the number the more zoomed in it is.
const BOTTOM_DROPDOWNS = true; // create dropdowns below map
...
function buildMap() { // builds the base state for the map
...
function eqfeed_callback(results) { // generate markers from locations.js file
...
function buildLeftList(location, index) {
  ...
  var newH4 = `<h4>${location.dealerName}</h4>`;
  var newWebsite = location.website ? `<span class='website'><a href='${location.website}' target='_blank'>${location.website}</a></span>` : "";
  var newPhone = location.phone ? `<a href='tel:${location.phone}' target='_blank' class='phone'>${location.phone}</a>` : "";
  var newAddress = `<span class='address'><i class='fa fa-map-marker'></i> ${location.address},  <span>${location.city}, ${location.state} ${location.zip}</span></span>`;
  newLi.innerHTML = `<div id='${index}'> ${newH4} ${newWebsite} ${newPhone} ${newAddress}</div>`;
...
function activateMarker(marker, location, coords) {
    google.maps.event.addListener(marker, "click", function(e) {
      ...
      var dealerAddress = `${location.city}, ${location.state}`;
      var position = {
          lat: coords[0],
          lng: coords[1]
      };
      ...
      var website = location.website ? `<a href='${location.website}' target='_blank' class='lead-button site-button'>Visit Site</a>` : "";
      var contentString = `<div class="dealership-info"><h3>${dealerName}</h3><p>${dealerAddress}</p> ${directions} ${website}</div>`;
...
function bounceAndZoom(zoom, marker, map, seconds) {
  ...
  setTimeout(function(){ marker.setAnimation(null); }, seconds);
}

function buildStateDropdowns(state_array, numLocations, results) {
  for (var i = 0; i < state_array.length; i++) {
    var state = state_array[i];
    var header = `<li class='panel state-${state}'><h4 id='toggle${state}' class='state-heading'><a class='locations-toggle collapsed' data-parent='#area-locations' data-toggle='collapse' href='#locations${state}'> <span>${state}</span></a></h4>`;
    var inner = `<div class='state-content collapse' id='locations${state}' role='tabpanel'><div><ul class='locations-list'></ul></div></div>`;
  ...
  for (var i = 0; i < numLocations; i++) {
    ...
    var innerWebsite = loc.website ? `<a href='${loc.website}' target='_blank'>${loc.dealerName}</a>` : loc.dealerName;
    var innerAddress = `<span class='address'><span>${loc.address}</span><span>${loc.city}, ${loc.state}&nbsp;${loc.zip}</span></span>`;
    var innerText = `<h4>${innerWebsite}</h4>${innerAddress}`;
    var innerPhone = loc.phone ?  `<span class='phone'><a href='tel:${loc.phone}'>${loc.phone}</a></span>` : "";
    var innerDirections = directionsLink(loc.dealerName,loc.address,loc.city,loc.state,loc.zip,"Get Directions","");
    $("#area-locations").find(`.${statename} .state-content > div > .locations-list`).append(`<li class='state-location'>${innerText} ${innerPhone} ${innerDirections}</li>`);
  }
...
function directionsLink(name, address, city, state, zip, text, class_name) {
  return `<a href="https://www.google.com/maps/dir/?api=1&destination=${name}&nbsp;${address}&nbsp;${city}&nbsp;${state}, ${zip}/@" class='${class_name}' target="_blank">${text}</a>`
}
