...
var YouTuber = function() {
    ...
    return _createClass(a, [{
        ...
        value: function updateDurations() {
            for (var b = this, c = 0; c < this.durationData.length; c++) 'undefined' != typeof b.thumbElems[c] && (b.thumbElems[c].querySelector('.card-header .duration') || (b.thumbElems[c].querySelector('.card-header').innerHTML += '<span class="duration">' + b.formatDuration(b.durationData[c].contentDetails.duration) + '</span>', b.showElem(b.thumbElems[c])))
        }
    }, {
        key: 'showElem',
        ...
        value: function createMarkup(b) {
            ...
            if (g.setAttribute('id', this.options.playerId), g.setAttribute('class', 'youtuber-video'), f.append(g), b.setAttribute('class', this.options.playerId), b.append(c), b.append(f), 'video' != this.options.type) {
                ...
                if (l.setAttribute('class', 'yt-loading'), l.style.opacity = '0', l.style.visibility = 'hidden', l.style.display = 'none', k.append(l), b.append(h), b.append(k), this.options.infiniteScroll) {
                    var m = document.createElement('button');
                    m.setAttribute('class', 'infinite-scroll load-more-btn'), m.innerText = 'Load More Videos', m.addEventListener('click', function() {
                        this.debounce(this.nextPage(), 250)
                    }.bind(this));
                    var n = document.createElement('button');
                    n.setAttribute('class', 'infinite-scroll back-to-top-btn'), n.innerHTML = '<i class="fa fa-arrow-up">&nbsp;</i>', n.style.opacity = '0', n.style.visibility = 'hidden', n.addEventListener('click', this.debounce(function() {
                        this.scrollToTop()
                    }.bind(this), 100)), window.addEventListener('scroll', this.throttle(function() {
                        this.checkPageScroll()
                    }.bind(this), 500)), b.append(m), b.append(n)
                }
            }
        }
    }, {
        ...
        value: function generateThumbnails(b) {
            if (b && 'video' != this.options.type) {
                ...
                if (!c.querySelector('.gallery.page-' + this.currentPage)) {
                    ...
                    for (var h = 0; h < b.length; h++) this.videoCount++, g.innerHTML += this.videoListsData[this.currentPage].items[h].contentDetails && this.videoListsData[this.currentPage].items[h].contentDetails.duration ? '<li style="opacity:0; visibility:hidden;" class="video-thumb gallery-item" data-id="' + b[h] + '"data-index="' + (this.videoCount - 1) + '"><div class="card"><div class="card-header"><img src="https://img.youtube.com/vi/' + b[h] + '/mqdefault.jpg" /><span class="duration">' + this.formatDuration(this.videoListsData[this.currentPage].items[h].contentDetails.duration) + '</span></div><div class="card-content"><span class="title">' + this.videoListsData[this.currentPage].items[h].snippet.title + '</span><span class="creation">' + this.formatDate(this.videoListsData[this.currentPage].items[h].snippet.publishedAt) + '</span><span class="description">' + this.videoListsData[this.currentPage].items[h].snippet.description + '</span></div></div></li>' : '<li style="opacity:0; visibility:hidden;" class="video-thumb gallery-item" data-id="' + b[h] + '"data-index="' + (this.videoCount - 1) + '"><div class="card"><div class="card-header"><img src="https://img.youtube.com/vi/' + b[h] + '/mqdefault.jpg" /></div><div class="card-content"><span class="title">' + this.videoListsData[this.currentPage].items[h].snippet.title + '</span><span class="creation">' + this.formatDate(this.videoListsData[this.currentPage].items[h].snippet.publishedAt) + '</span><span class="description">' + this.videoListsData[this.currentPage].items[h].snippet.description + '</span></div></div></li>';
        ...
        value: function goToPage(b) {
            ...
            var f = setInterval(function() {
                ...
                if (b == c.currentPage) c.paginationChanging = !1, c.updateCurrentPage(), clearInterval(f);
                    ...
                    c.currentPage = parseInt(b), '1' != h[b - 1].style.opacity && (h[b - 1].style.opacity = '1'), 'visible' != h[b - 1].style.visibility && (h[b - 1].style.visibility = 'visible'), 'block' != h[b - 1].style.display && (h[b - 1].style.display = 'block');
